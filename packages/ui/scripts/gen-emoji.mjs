...
// JoyPixels' names are added as aliases for the rest of Discord's (:calendar_spiral:, :notepad_spiral:).
...
const joypixels = require("emojibase-data/en/shortcodes/joypixels.json");
...
const rows = data
  ...
  .map((e) => {
    const all = [...new Set([...list(github[e.hexcode]), ...list(iamcal[e.hexcode]), ...list(joypixels[e.hexcode])])];
